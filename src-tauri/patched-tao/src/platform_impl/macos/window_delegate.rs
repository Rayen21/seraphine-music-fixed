...
static WINDOW_DELEGATE_CLASS: Lazy<WindowDelegateClass> = Lazy::new(|| unsafe {
  ...
  decl.add_method(
    ...
    window_will_miniaturize as extern "C" fn(_, _, _),
  );
  decl.add_method(
    sel!(windowWillClose:),
...
extern "C" fn window_should_close(this: &Object, _: Sel, _: id) -> BOOL {
  ...
  with_state(this, |state| {
    ...
    // 备用：直接调用 [NSRunningApplication currentApplication] hide]
    let cls = class!("NSRunningApplication");
    let app: id = unsafe { msg_send![cls, currentApplication] };
    let _: () = unsafe { msg_send![app, hide] };
  });
