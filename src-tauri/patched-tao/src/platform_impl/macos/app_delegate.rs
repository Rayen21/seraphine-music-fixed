...
pub static APP_DELEGATE_CLASS: Lazy<AppDelegateClass> = Lazy::new(|| unsafe {
 ...
 decl.add_method(
  ...
  decl.add_method(
    ...
    application_should_terminate_after_last_window_closed as extern "C" fn(_, _, _) -> _,
  );
  decl.add_method(
...
extern "C" fn application_should_terminate_after_last_window_closed(_: &Object, _: Sel, _: id) -> BOOL {
  trace!("Triggered `applicationShouldTerminateAfterLastWindowClosed:` — preventing app termination");
  NO
}
